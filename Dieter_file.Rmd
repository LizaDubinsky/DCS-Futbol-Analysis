...
```{r}
from_table <-table(france_semi[,2])
...
barplot(df_from_semi_ordered$Freq,decreasing = TRUE, names.arg = df_from_semi_ordered$Var1, decreasing = TRUE, ylab = "Number of passes from", xlab = "Player" ,col = mycolors[df_from_semi_ordered$Position],las=2,cex.names = 0.5, legend.text= levels(df_from_semi_ordered$Position), args.legend = list("topright", fill =mycolors ))

'
, legend.text =levels(df_from_semi_ordered$Position), args.legend = mycolors
library(graphics)
#sw.countdf<-as.data.frame(sw.count)
maxd <- max(df_from_semi$Freq)
usersh<-hist(df_from_semi$Freq, breaks=8, xlab = "Number of Women at an event", ylab = "Number of events with the given attendance", main = "Distribution of Attendees per Event")
'
```
