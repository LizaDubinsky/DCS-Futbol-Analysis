...
## Final project for DCS 375 on Players in the 2018 World Cup Semifinal and Final


### Introduction 

In this project, we used Statsbomb event data from the [Dato-Futbol/passing-networks GitHub repository](https://github.com/Dato-Futbol/passing-networks) to analyze the final and semifinal games in the 2018 World Cup. Motivated by scholarly articles on constructing and analyzing passing networks, we used network analysis to explore the following questions:

1. What players are central to the directed networks of the France-Croatia final and France-Belgium semifinal? What positions do they play?
2. How are the number of passes allocated across players on each team? 


### Background 

The most popular sport in the world brings forth a complexity when looking beyond the score and using methods, metrics, and visualization to analyse a game. There are many events that occur in one soccer match such as shots, goals, passes, fouls, among many others. There are multitude amounts of statistics that are be used to analyse a player, a position, or a whole team. There a variety of competitions national teams can participate in. The World Cup, played ever 4 years, is the biggest stage to play for any soccer player and national team. We choose to analyze the current reigning champions of the world, France, and their trajectory through the semi-final game and final game as well as their opponents in these games. There has been a total of 21 World cups France has qualified 15 times to the competition. France has made it to the semifinals or further in 6 occasions and winning it twice. Currently they are considered one of the best teams, being ranked 3rd in the FIFA men's ranking. 

In many soccer games a man of the match is selected and it usually goes to a player that effectively contributed the most in the game. The contribution of the player can range from scoring a decisive goal to a goalkeeper keeping a clean sheet after many chances from the counter-part. Using this idea of MOTM, we want to explore the team and it's player's significance in the game based on their number of passes. Passing networks are an example to mathematically and visually analyze what happens in the pitch and how players interact with each other throughout the game. Many times key players are not the ones who score the goals, but the ones that allow for the creation of scoring chances. This project studies individual player's passing contributions to further understand a team's dynamic and the players themselves. We do not take into account current position of a player at the moment of the pass or it’s difficulty, but rather it’s overall presence with the ball and with it's teammates. 
...
The original dataset included 227886 rows and 161 columns that covered the games of the 2018 World Cup. We decided to focus our analysis on the final and semifinal games of the 2018 World Cup, and filtered for variables concerning the teams, players, and type of play:
...
The below code allowed us to isolate only the passing plays for our analyses, as well as create 2 datasets and write 2 .csv files for the final and semifinal games:

```{r filter-data-write-csv, warning = FALSE}
# grab only the passes from the types of plays
network_passes <- subset(wc_df_official, type.name == "Pass")

# create 2 data sets for 2 games we plan to analyze by taking subset of original dataset for two teams per game
...
# remove players who did not play for france in the final
...
```{r extra-france-final-filter, include = FALSE}
...
```{r connected-pass-french-final-filter, warning = FALSE}
# construct french final dataset for connected passes only 
connected_pass_france_final <- subset(france_only_final, is.na(pass.outcome.name))
```

```{r croatia-network-object, include = FALSE}
# remove players who did not play for croatia in the final
...
croatia_final.g <- graph.edgelist(croatia_final, directed = TRUE)
```
...
# remove players who didn't play for france or belgium in the semifinal
...
```{r france-final-network-object, warning = FALSE}
# create an edgelist of passes to and from players
...
To create accessible network visualizations, we used shape and color to distinguish between the positions played by player in each team across both games. The goalkeeper is a yellow sphere, the defenders are blue circles, the midfielders are purple squares, and the forwards are green rectangles. The below code assigns players on the French team to different positions and defines the color and shape of each player node based on their position:

```{r identify-and-color-france-players, warning = FALSE}
# create vectors to identify French players based on their position 
...
# specify color and shape of player nodes based on their position 
...
# create vectors to identify Croatian players based on their position 
...
```{r identify-belgium-players, include = FALSE}
# create vectors to identify Belgian players based on their position 
...
```{r color-croatia-belgium-french-semi-nodes, include = FALSE, warning = FALSE}
...
# changing color and shape of french semifinal nodes to represent a specific position
...
# changing color and shape of belgium semifinal nodes to represent a specific position
...
```{r set-seed-write-french-croatia-files, warning = FALSE, message = FALSE}
# set specific seed to get the same shape of the network every time and write network file for france final passing network
set.seed(11)
...
# set specific seed to get the same shape of the network every time and write network file for croatia final passing network
set.seed(10)
...
```{r final-soccer-field-network-setup, warning = FALSE, message = FALSE}
# read in soccer field background and network files for french and croatian passing networks
...
# scaling french and croatian passing network images to 1000 height
france_png_final <- image_scale(france_png_final, "x1000")
croatia_png_final <- image_scale(croatia_png_final, "x1000")

#creating a composite of both images and moving the french and croatian networks using offset to the right and down
...
```{r set-seed-write-french-belgium-files, include = FALSE, message = FALSE}
# set specific seed to get the same shape of the network every time and write network file for belgium semifinal passing network
set.seed(2)
...
# set specific seed to get the same shape of the network every time and write network file for french semifinal passing network
set.seed(1)
...
```{r semi-soccer-field-network-setup, include = FALSE}
...
#scaling France and Belgium passing networks images to 1000 height
france_pgn_semi <- image_scale(france_pgn_semi, "x1000")
belgium_png_semi  <- image_scale(belgium_png_semi , "x1000")

#creating a composite of both images and moving the France and Belgium networks using offset to the right and down
# scaling the soccer field to 1000 in height 
...
##### France in the 2018 World Cup Semifinal 

```{r plot-france-semi-field-network, warning = FALSE}
# plot the france passing network from the World Cup semifinal on soccer field
plot(semi_france_image)
```

The above plot is a directed network showing the passes  from and to each player. We can observe that a great concentration of edges between Pavard, Mbappe, Pogba, and Varane. These players represent the right side of the french lineup and are considered a lot more attacking that its left side. The attacks coming from this side is not surprising because in the the right side, Pavard is a very offensive defensive player that participates in sending crosses or connecting with strikers. This is shown with by amount of passes with Mbappe and Pogba. In comparison, french’s left side consisting of defender Hernandez and Umtiti have a much lower connection with the forwards. They mostly pass with each other or the center midfielders. The lack of connection between Varane , Pavard and Umtiti Hernandez shows that during this game the defenders did not swing the ball much in the back. This can due to the fact that the Belgian team was attacking and pressing quite bit. 

##### Belgium in the 2018 World Cup Semifinal 

```{r plot-belgium-semi-field-network, warning = FALSE}
# plot the belgium passing network from the World Cup semifinal on soccer field
plot(semi_belgium_image)
```

The directed passing network above represents the Belgium’s passing from and to each player. Belgium had a significantly greater amount of possession than France. This can be seen with the passing among their defenders and midfield. However, they lacked an attacking component having fewer shots than France and its reflected with the lack of passing to the forwards. Additionally, De Bruyne is has an important presence in the attacking, but this game the passes he completed did not go as forward as the usual. He was restrained to passing through the middle. This can possibly be granted to the French emphasis on shutting him down quickly and forcing him to play back.

##### France in the 2018 World Cup Final 

```{r plot-france-final-field-network, warning = FALSE}
# plot the france passing network from the World Cup final on soccer field
plot(final_france_image)
```

The plot above represents Frances passing network in the world cup final. Similar to the semifinal Varane, Pavard, Mbape, and Pogba have significant amount of connections. Additionally, Kante also has a lot of connections with everyone. This shows how France continues to attack to from the right side of the field. Though during this game the left side defenders, midfields and forwards have a greater number of connections between. It seems like they increase their attack through the left side which corresponding to 4 goals, compared to only one in the semifinal. 


##### Croatia in the 2018 World Cup Final 

```{r plot-croatia-final-field-network, warning = FALSE}
# plot the croatia passing network from the World Cup final on soccer field
plot(final_croatia_image)
```

The directed passing network above represents Croatia’s passing from and to each player. Croatia had a significant amount of possession  during the game and this can be seen in the network with he defenders and midfielders having significant connections with each other. However, when it came to moving the ball forward the number of connections drops. The lack of passes to the forward can be attributed to the French defensive style. They allow for the other team to have possession in their own half, but Croatia crossed to the French side they would  press and prevent passes getting to their forwards. Modric seems to have the most diverse connections which makes sense since he usually controls the midfield for the Croatian team.

#### Betweenness Centrality and Closeness Centrality Statistics for Teams in the 2018 World Cup Semifinal and Final

The importance of  a player to a team is hard to define. Every player plays a crucial role on the team; the success of the team depends on the individual contributions of each player. Although the performance of the team depends on every player, some players are more influential and contribute more than other players. The influence of players on the teams’ accomplishments depends on different factors. For instance,  the position they play will have an impact on how much they interact with other players and the ball, thus, determining their influence during the game. 

From [UNGAR Chapter 7: Actor Prominence](https://link.springer.com/book/10.1007/978-3-319-23883-8), we will use measures of centrality to determine the prominence of players on the teams in the 2018 World Cup semifinal and final. We will use closeness centrality which measures how close a given node is to other nodes in the network as well as betweenness centrality which measures to what extent a given node is positioned "between" pairs of other nodes in the network.

To determine which players were central to the directed networks of the France-Croatia final and France-Belgium semifinal, we referred to the [2012 article "A network theory analysis of football strategies" by Javier López Peña and Hugo Touchette](https://arxiv.org/pdf/1206.6904.pdf). In their article, they use closeness centrality and betweenness centrality to measure a player's contribution to the team. Guided by their insights, we will use betweenness centrality to inform the extent to which ball-flow between the rest of the team depends on each player and closeness centrality to inform how well-connected each player. 

##### Betweenness Centrality and Closeness Centrality for French Players in the Semifinal

```{r france-semi-betweenness, warning = FALSE}
# betweenness for france semifinal
btwn.france_semi <- betweenness(france_semi.g) # calculating betweenness for france semifinal graph object
names(btwn.france_semi) <- V(france_semi.g)$name # grabbing player names from france semifinal graph object
ind <- order(-btwn.france_semi) # reordering in descending order 
btwn.france_semi[ind][1:13] # displaying table of france semifinal betweenness centrality scores
``` 

We observe here that Kanté, Pogba, and Varane have the largest betweenness centrality scores among French players in the semifinal with scores of 12.027, 7.544, and 5.650 respectively. This means that the impact of removing these three players from the 2018 World Cup semifinal would have a far greater effect than removing players like Sidibé with a betweenness centrality score of less than 0.1. These top three players are two midfielders and a defender for France. Further, from the large variation in betweenness scores, we see that there is a high dependence on a few important players in the French passing network for the France-Belgium semifinal.

```{r france-semi-closeness, warning = FALSE}
# closeness for france semifinal
closeness.france_semi <- closeness(france_semi.g) # calculating closeness for france semifinal graph object
names(closeness.france_semi) <- V(france_semi.g)$name # grabbing player names from france semifinal graph object
ind <- order(-closeness.france_semi) # reordering in descending order 
closeness.france_semi[ind][1:13] # displaying table of france semifinal closeness centrality scores
```

From our closeness centrality calculation, we see that Kanté, Griezmann, and Varane have the largest closeness centrality scores among French players in the semifinal with scores of 0.083, 0.077, and 0.071 respectively. Since closeness centrality is calculated out of 1, these are still very small scores that measure how well-connected a player is based on how easy it is for other players to reach them. These top three players are a midfielder, forward, and defender respectively.

From these two calculations of centrality, midfielder N''Golo Kanté is clearly the most central to the passing network of the French team in the 2018 World Cup semifinal, followed closely by other key players including defender Varane, midfielder Pogba, and forward Griezemann. 


##### Betweenness Centrality and Closeness Centrality for Belgian Players in the Semifinal

```{r belgium-semi-betweenness, warning = FALSE}
# betweenness for belgium semifinal 
btwn.belgium_semi <- betweenness(belgium_semi.g) # calculating betweenness for belgium semifinal graph object 
names(btwn.belgium_semi) <- V(belgium_semi.g)$name # grabbing player names from belgium semifinal graph object
ind <- order(-btwn.belgium_semi) # reordering in descending order 
btwn.belgium_semi[ind][1:14] # displaying table of france semifinal betweenness centrality scores
``` 

We observe here that Hazard, Meunier, and Witsel have the largest betweenness centrality scores among Belgian players in the semifinal with scores of 9.520, 8.983, and 8.779 respectively. This means that the impact of removing these three players from the 2018 World Cup semifinal would have a far greater effect than removing players like Tunga with a betweenness centrality score of less than 0.3. These top three players are a forward, a defender, and a midfielder respectfully. Further, from the large variation in betweenness scores, we see that there is a high dependence on a few important players in the Belgian passing network for the France-Belgium semifinal.

```{r belgium_semi-closeness, warning = FALSE}
# closeness for belgium semifinal 
closeness.belgium_semi <- closeness(belgium_semi.g) # calculating closeness for belgium semifinal graph object 
names(closeness.belgium_semi) <- V(belgium_semi.g)$name # grabbing player names from belgium semifinal graph object
ind <- order(-closeness.belgium_semi) # reordering in descending order
closeness.belgium_semi[ind][1:14] # displaying table of belgium semifinal closeness centrality scores
```

From our closeness centrality calculation, we see that Witsel, De Bruyne, and Hazard have the largest closeness centrality scores among Belgian players in the semifinal with scores of 0.071, 0.067, and 0.067 respectively. Since closeness centrality is calculated out of 1, these are still very small scores that measure how well-connected a player is based on how easy it is for other players to reach them. These top three players are two midfielders and a forward respectively.

From these two calculations of centrality, forward Eden Hazard and midfielder Axel Witsel are the most central to the passing network of the Belgian team in the 2018 World Cup semifinal, followed closely by other key players including midfielder De Bruyne. 

##### Betweenness Centrality and Closeness Centrality for French Players in the Final

```{r france-final-betweenness, warning = FALSE}
# betweenness for france final
btwn.france_final <- betweenness(france_final.g) # calculating betweenness for france final graph object 
names(btwn.france_final) <- V(france_final.g)$name # grabbing player names from france final graph object
ind <- order(-btwn.france_final) # reordering in descending order
btwn.france_final[ind][1:14] # displaying table of france final betweenness centrality scores
``` 

We observe here that Pavard, Kanté, and Pogba have the largest betweenness centrality scores among French players in the final with scores of 16.599, 9.316, and 5.041 respectively. This means that the impact of removing these three players from the 2018 World Cup final would have a far greater effect than removing players like Fekir with a betweenness centrality score of less than 0.4. These top three players are a defender and two midfielders respectfully. Further, from the large variation in betweenness scores, we see that there is a high dependence on a few important players in the French passing network for the France-Croatia final.

```{r france-final-closeness, warning = FALSE}
# closeness for france final
closeness.france_final <- closeness(france_final.g) # calculating closeness for france final graph object 
names(closeness.france_final) <- V(france_final.g)$name # grabbing player names from france final graph object
ind <- order(-closeness.france_final) # reordering in descending order
closeness.france_final[ind][1:14] # displaying table of france final closeness centrality scores
```

From our closeness centrality calculation, we see that Kanté, Pogba, and Pavard have the largest closeness centrality scores among French players in the final with scores of 0.077, 0.071, and 0.067 respectively. Since closeness centrality is calculated out of 1, these are still very small scores that measure how well-connected a player is based on how easy it is for other players to reach them. These top three players are two midfielders and a defender respectively.

From these two calculations of centrality, midfielders N''Golo Kanté and Paul Pogba as well as defender Benjamin Pavard are the most central to the passing network of the French team in the 2018 World Cup final, followed closely by other key players including defenders Umtiti, Varane, and Pi as well as forward Griezmann. 

##### Betweenness Centrality and Closeness Centrality for Croatian Players in the Final

```{r croatia-final-betweenness, warning = FALSE}
# betweenness for croatia final
btwn.croatia_final <- betweenness(croatia_final.g) # calculating betweenness for croatia final graph object 
names(btwn.croatia_final) <- V(croatia_final.g)$name # grabbing player names from croatia final graph object
ind <- order(-btwn.croatia_final) # reordering in descending order
btwn.croatia_final[ind][1:13] # displaying table of croatia final betweenness centrality scores
``` 

We observe here that Kramarić, Modrić, and Lovren have the largest betweenness centrality scores among Croatian players in the final with scores of 11.734, 10.598, and 6.624 respectively. This means that the impact of removing these three players from the 2018 World Cup final would have a far greater effect than removing players like Pjaca with a betweenness centrality score of less than 0.01. These top three players are a forward, a midfielder, and a defender respectfully. Further, from the large variation in betweenness scores, we see that there is a high dependence on a few important players in the Croatian passing network for the France-Croatia final.

```{r croatia-final-closeness, warning = FALSE}
# closeness for croatia final
closeness.croatia_final <- closeness(croatia_final.g) # calculating closeness for croatia final graph object 
names(closeness.croatia_final) <- V(croatia_final.g)$name # grabbing player names from croatia final graph object
ind <- order(-closeness.croatia_final) # reordering in descending order
closeness.croatia_final[ind][1:13] # displaying table of croatia final closeness centrality scores
```

From our closeness centrality calculation, we see that Modrić, Brozović, and Rakitić have the largest closeness centrality scores among Croatian players in the final with scores of 0.077, 0.077, and 0.071 respectively. Since closeness centrality is calculated out of 1, these are still very small scores that measure how well-connected a player is based on how easy it is for other players to reach them. These top three players are midfielders for Croatia.

From these two calculations of centrality, midfielder Luka Modrić is the most central to the passing network of the Croatian team in the 2018 World Cup final, followed closely by other key players including midfielder Rakitić and defender Lovren.

Midfielders are constantly interacting with the ball; they are the connections between the defenders and the forward players. They dictate how much the team is moving back and forth on the field. Although defenders don’t interact with the ball as much as the midfielders do, they have sufficient interactions with the ball that it would show. Their job is to stop attacks from the opposing team and prevent the opposing team from scoring a goal. Ultimately, it suffices to conclude that some of the most valuable players to a team will be players who interact the most with the ball and well as other players. In this case, the midfielders and defenders are invaluable to the three teams that played in the 2018 Word Cup semifinal and final. 

#### Modeling Sent and Received Passing Distributions for Teams in the 2018 World Cup Final and Semifinal
...
```{r create-frequency-tables-france-croatia-final, warning = FALSE, message = FALSE}
france_final_passes_from <- table(france_final[, 1]) # creating frequency table from players in passes from column in france_final dataset
france_final_passes_to <- table(france_final[, 2]) # creating frequency table from players in passes to column in france_final dataset

croatia_final_passes_from <- table(croatia_final[, 1]) # creating frequency table from players in passes from column in croatia_final dataset
croatia_final_passes_to <- table(croatia_final[, 2]) # creating frequency table from players in passes to column in croatia_final dataset
```
...
# plotting barplot of number of passes sent by french players in semifinal, colored based on the position of the player
barplot(df_from_semi_france_sent_ordered$Freq,decreasing = TRUE, names.arg = df_from_semi_france_sent_ordered$Var1, decreasing = TRUE, ylab = "Number of Passes" ,col = mycolors[df_from_semi_france_sent_ordered$Position],las=2,cex.names = 0.5, legend.text= levels(df_from_semi_france_sent_ordered$Position), args.legend = list("topright", fill =mycolors ),  main = "Passes Sent in the 2018 World Cup Semifinal (France)")
```

From the barplot of sent passes by players on the French team, we observe that midfielders Kanté and Pogba as well as defenders Varane and Pavard make up 254 of the 497 sent passes. This means that 4 of the 13 players make up over 50% of the sent passes by players on the French team in the France-Belgium semifinal.
...
# plotting barplot of number of passes received by french players in semifinal, colored based on the position of the player
barplot(df_to_semi_france_received_ordered$Freq,decreasing = TRUE, names.arg = df_to_semi_france_received_ordered$Var1, decreasing = TRUE, ylab = "Number of Passes" ,col = mycolors[df_to_semi_france_received_ordered$Position],las=2,cex.names = 0.5, legend.text= levels(df_to_semi_france_received_ordered$Position), args.legend = list("topright", fill = mycolors ),  main = "Passes Received in the 2018 World Cup Semifinal (France)")
```

From the barplot of received passes by players on the French team, we observe that midfielders Pogba and Kanté as well as defender Pavard and forward Mbappé Lottin make up 235 of the 497 received passes. This means that 4 of the 13 players make up 47% of the received passes by players on the French team in the France-Belgium semifinal.

We ran a power law fit test to determine if the distribution of sent and received passes on the French team in the French-Belgium semifinal was that of a power law distribution. We calculated an alpha value and p-value using the code below:

```{r power-law-fit-test-french-semi, warning = FALSE, message = FALSE}
# Creates Frequency Tables by Player for the French Team in the Semifinal
...
# plotting barplot of number of passes sent by belgium players in semifinal, colored based on the position of the player
barplot(df_from_semi_bel_ordered$Freq,decreasing = TRUE, names.arg = df_from_semi_bel_ordered$Var1, decreasing = TRUE, ylab = "Number of Passes" ,col = mycolors[df_from_semi_bel_ordered$Position],las=2,cex.names = 0.5, legend.text= levels(df_from_semi_bel_ordered$Position), args.legend = list("topright", fill =mycolors ),  main = "Passes Sent in the 2018 World Cup Semifinal (Belgium)")

```

From the barplot of sent passes by players on the Belgian team, we observe that defenders Alderweireld and Vertonghen as well as midfielder Witsel and forward Hazard make up 274 of the 569 sent passes. This means that 4 of the 14 players make up 48% of the sent passes by players on the Belgian team in the France-Belgium semifinal.
...
# plotting barplot of number of passes received by belgium players in semifinal, colored based on the position of the player
barplot(df_to_semi_bel_ordered$Freq,decreasing = TRUE, names.arg = df_to_semi_bel_ordered$Var1, decreasing = TRUE, ylab = "Number of Passes" ,col = mycolors[df_to_semi_bel_ordered$Position],las=2,cex.names = 0.5, legend.text= levels(df_to_semi_bel_ordered$Position), args.legend = list("topright", fill =mycolors ),  main = "Passes Received in the 2018 World Cup Semifinal (Belgium)")
```

From the barplot of received passes by players on the Belgian team, we observe that defenders Alderweireld and Vertonghen as well as forward Hazard and midfielder Witsel make up 278 of the 569 sent passes. This means that 4 of the 14 players make up 49% of the sent passes by players on the Belgian team in the France-Belgium semifinal.

Then, we ran a power law fit test to determine if the distribution of sent and received passes on the Belgian team in the French-Belgium semifinal was that of a power law distribution. We calculated an alpha value and p-value using the code below:
...
# plotting barplot of number of passes sent by french players in final, colored based on the position of the player
...
From the barplot of sent passes by players on the French team, we observe that midfielders Kanté and Pogba as well as defenders Pavard and Varane make up 246 of the 513 sent passes. This means that 4 of the 14 players make up 48% of the sent passes by players on the French team in the France-Croatia final.
...
# plotting barplot of number of passes received by french players in final, colored based on the position of the player
...
From the barplot of received passes by players on the French team, we observe that midfielders Kanté and Pogba as well as defender Pavard and forward Mbappé Lottin make up 228 of the 513 sent passes. This means that 4 of the 14 players make up 44% of the received passes by players on the French team in the France-Croatia final.
...
# plotting barplot of number of passes sent by croatian players in final, colored based on the position of the player
...
From the barplot of sent passes by players on the Croatian team, we observe that midfielders Modrić, Rakitić, and Brozović as well as defender Lovren make up 347 of the 628 sent passes. This means that 4 of the 13 players make up 55% of the sent passes by players on the Croatian team in the France-Croatia final.
...
# plotting barplot of number of passes received by croatian players in final, colored based on the position of the player
...
From the barplot of received passes by players on the Croatian team, we observe that midfielders Modrić and Rakitić as well as defenders Lovren and Vrsaljko make up 302 of the 628 sent passes. This means that 4 of the 13 players make up 48% of the sent passes by players on the Croatian team in the France-Croatia final.
...
The higher centrality scores as well as the high number of passes sent and received by midfielders and defenders on each team across both games verifies the findings from the 2017 article [Midfielder as the prominent participant in the building attack: A network analysis of national teams in FIFA World Cup 2014 by Clemente, Filipe Manuel et al.](https://www.tandfonline.com/doi/abs/10.1080/24748668.2015.11868825) on the 2014 World Cup that midfielders and defenders are prominent nodes in passing networks using degree prestige, prestige, and betweenness centrality. A 2020 article [Player position relationships with centrality in the passing network of world cup soccer teams: Win/loss match comparisons also by Clemente, Filipe Manuel et al.](https://www.sciencedirect.com/science/article/pii/S0960077920300242#bib0007) concludes that midfielders and defenders are critical links in the passing network. 
 
#### Limitations and Future Work

While these passing network analyses offer insights into playing strategy and key players, there are multiple ways that these analyses may be refined or extended in the future. By standardizing by playing time, our analyses of pass distributions across teams in different 2018 World Cup games would more accurately reflect team dynamics and key player positions in sending and receiving passes on different teams. Further, by utilizing the starting and ending coordinates of passes from the [Dato-Futbol/passing-networks GitHub repository](https://github.com/Dato-Futbol/passing-networks), our passing network visualizations could be extended from what the article [Using Network Science to Analyse Football Passing Networks: Dynamics, Space, Time, and the Multilayer Nature of the Game by Buldú, Javier M. et al.](https://www.frontiersin.org/articles/10.3389/fpsyg.2018.01900/full) calls player passing networks where the network nodes are players on a team to pitch networks, where network nodes are regions on the field connected through passes made by players on the field positioned at these different regions. This type of network would lend itself to creating a network of player positions and accurate mappings of passes between them based on real-time coordinates data. It would even be possible to construct a heatmap of where passes occur across the field. 

#### References 

1. Nepusz, T., &amp; Csardi, G. (n.d.). Fit_power_law: Fitting a power-law distribution function to discrete data. RDocumentation. Retrieved April 19, 2022, from (https://www.rdocumentation.org/packages/igraph/versions/1.3.0/topics/fit_power_law)  
2. Luke, Douglas. 2015. A User's Guide to Network Analysis in R. (https://link.springer.com/book/10.1007/978-3-319-23883-8)
3. Peña, Javier López and Hugo Touchette. 2012. A network theory analysis of football strategies. (https://arxiv.org/pdf/1206.6904.pdf)
4. Buldú, Javier M. et al. 2018. Using Network Science to Analyse Football Passing Networks: Dynamics, Space, Time, and the Multilayer Nature of the Game. (https://www.frontiersin.org/articles/10.3389/fpsyg.2018.01900/full)
5. Clemente, Filipe Manuel et al. 2017. Midfielder as the prominent participant in the building attack: A network analysis of national teams in FIFA World Cup 2014. (https://www.tandfonline.com/doi/abs/10.1080/24748668.2015.11868825)
6. Clemente, Filipe Manuel et al. 2020. Player position relationships with centrality in the passing network of world cup soccer teams: Win/loss match comparisons. (https://www.sciencedirect.com/science/article/pii/S0960077920300242#bib0007)