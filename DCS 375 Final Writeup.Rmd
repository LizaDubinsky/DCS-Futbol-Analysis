...
In the following sections, we will include our methodology for our analyses followed directly by our discussion of the relevant results and visualizations. 
...
```{r france-final-object, warning= FALSE}
...
```{r plot-soccer-field-networks}
plot(final_france_image)
plot(final_croatia_image)
```

 (include code snippets and interpretation)
...
To explore the distribution of sent and received passes on each team in both games, we first constructed frequency tables of the number of passes sent and received per player.

```{r create-frequency-tables, include = FALSE}
...
where the color of each bar corresponds to the player's position.
