...
To create accessible network visualizations, we used shape and color to distinguish between the positions played by player in each team across both games. The below code is for the French team in the France-Croatia final:
...
# so we dont have to re-read images from above
france_semi_png1 <- france_semi_png
soccer_field <-soccer_png

#scaling france network of passes image to 1000 height
france_semi_png1 <- image_scale(france_semi_png1, "x1000")
#creating a composite of both images and moving the France network using offset to the right and down
# scaling the soccer field to 1000 in height 
semi_france_image <- image_composite(image_scale(soccer_field, "x1000"), france_semi_png1, offset = "+500+0")

```

```{r plot-soccer-field-networks, warning = FALSE}
...
Then, we constructed bar plots, where the color of each bar corresponds to the player's position. The below code is for the passes recieved and sent by players on the French team in the France-Croatia final:
...
```{r france-semi-sent-passes-plot}
# plotting barplot of the frequency a player completed a pass, colored based on the positon of the player
...
```{r croatia-final-sent-passes-plot, warning = FALSE}
# plotting barplot of the frequency a player completed a pass, colored based on the positon of the player
...
```{r croatia-final-received-passes-plot, warning = FALSE}
# plotting barplot of the frequency a player completed a pass, colored based on the positon of the player
barplot(df_to_final_croatia_receieved_ordered$Freq,decreasing = TRUE, names.arg = df_to_final_croatia_receieved_ordered$Var1, decreasing = TRUE, ylab = "Number of Passes" ,col = mycolors[df_to_semi_bel_ordered$Position],las=2,cex.names = 0.5, legend.text= levels(df_to_semi_bel_ordered$Position), args.legend = list("topright", fill =mycolors ),  main = "Passes Received in the 2018 World Cup Final (Croatia)")
...
df_from_semi_france_sent <- as.data.frame(from_table)
...
```{r france-semi-sent-passes-plot, warning = FALSE}
...
```{r belgium-semi-sent-passes-plot}
# plotting barplot of the frequency a player completed a pass, colored based on the positon of the player
...
```{r belgium-semi-received-passes-plot, warning = FALSE}
# plotting barplot of the frequency a player completed a pass, colored based on the positon of the player
...
standardize by playing time, calculations like passes receieved and sent might look different if we have passes/min 
